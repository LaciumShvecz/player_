...
export interface IArtist { id: number; name: string; image: string; listenersCount: number }
export interface ITrack { id: number; name: string; file: string; cover: string; duration: number; artist: IArtist }
...
class TracksStore {
	...
	async fetchTracks() {
		const res = await axios.get<ITrack[]>('/api/tracks')
